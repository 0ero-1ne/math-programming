...
namespace combi
{
    ...
    unsigned __int64 factorial(unsigned __int64 x) { return (x == 0) ? 1 : (x * factorial(x - 1)); };
    unsigned __int64 permutation::count() const { return factorial(this->n); }
    ...
    unsigned __int64 factor(unsigned __int64 x) { return (x == 0) ? 1 : (x * factor(x - 1)); };
